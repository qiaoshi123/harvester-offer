...
19.array.toString()为啥会返回逗号隔开的字符串
...
连续重复的字符去重,这种会有重复项，aabbcccdc=>字符传替换成abcdc
reduce拼接，
function switchStr(str) {
    let arr = str.split('');
...
12.用二分查找递归方法，插入一个值并返回索引；[1,2,3,5,7,8],插入4，返回索引3；
```javascript
...
2.dom事件中 content.loaded和load的触发实际是什么 ？图片加载完成后是一个什么情况？
